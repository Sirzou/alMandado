defmodule AlMandadoWeb.UsuarioController do
  ...
  def index(conn, _params) do
    usuarios = Fronts.list_usuarios()
    render(conn, "index.html", usuarios: usuarios)
  end

  def new(conn, _params) do
    changeset = Fronts.change_usuario(%Usuario{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"usuario" => usuario_params}) do
    case Fronts.create_usuario(usuario_params) do
      {:ok, usuario} ->
        conn
  ...
  def update(conn, %{"id" => id, "usuario" => usuario_params}) do
    ...
    case Fronts.update_usuario(usuario, usuario_params) do
      {:ok, usuario} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    usuario = Fronts.get_usuario!(id)
    {:ok, _usuario} = Fronts.delete_usuario(usuario)
